...
private struct ProgressPage: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 18) {
                ...
                chartCard(title: "Emotional calm score") {
                    Chart(calmTrend) { item in
                        LineMark(
                            x: .value("Day", item.day),
                ...
                chartCard(title: "Pattern insights") {
                    HStack(spacing: 12) {
                        Image(systemName: "lightbulb.fill")
                            .foregroundStyle(PulzPalette.petrol)
                        Text("Your most vulnerable window is between 13:00–15:00. Plan a stabilizing step then.")
                            .font(PulzType.body(13))
                            .foregroundStyle(PulzPalette.text)
                    }
                }
    ...
    private var weeklyEpisodes: [ChartSample] {
        [
            ChartSample(day: "Mon", value: 2),
            ChartSample(day: "Tue", value: 4),
            ChartSample(day: "Wed", value: 1),
            ChartSample(day: "Thu", value: 5),
            ChartSample(day: "Fri", value: 3),
            ChartSample(day: "Sat", value: 2),
            ChartSample(day: "Sun", value: 1)
        ]
    }

    private var calmTrend: [ChartSample] {
        [
            ChartSample(day: "Mon", value: 48),
            ChartSample(day: "Tue", value: 52),
            ChartSample(day: "Wed", value: 58),
            ChartSample(day: "Thu", value: 49),
            ChartSample(day: "Fri", value: 67),
            ChartSample(day: "Sat", value: 63),
            ChartSample(day: "Sun", value: 72)
        ]
    }

    private var triggerStats: [TriggerSample] {
        [
            TriggerSample(name: "Work stress", value: 8),
            TriggerSample(name: "Loneliness", value: 6),
            TriggerSample(name: "Fatigue", value: 5),
            TriggerSample(name: "Boredom", value: 4),
            TriggerSample(name: "Conflict", value: 3)
        ]
    }
    ...
    private struct TriggerSample: Identifiable {
        let id = UUID()
        let name: String
        let value: Double
    }
...
private struct MyPulzView: View {
    ...
    @State private var urgeMessage = "Let's pause together. One breath at a time."
    @State private var recoveryMessage = "You got through it. That took strength."
    @State private var vibrationPattern = "Gentle pulse"
    @State private var vibrationIntensity: Double = 3
    ...
    @State private var selectedTone = "Warm & nurturing"
    @State private var interventionLanguage = "Romanian"
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 18) {
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    messageField(title: "WHEN YOU NEED GROUNDING", text: $groundingMessage)
                    messageField(title: "WHEN AN URGE IS DETECTED", text: $urgeMessage)
                    messageField(title: "AFTER A DIFFICULT MOMENT", text: $recoveryMessage)
                }
                .glassCard(accent: PulzPalette.lavender)
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    HStack(spacing: 10) {
                        selectionPill(title: "Gentle pulse", subtitle: "Two soft vibrations", selected: vibrationPattern == "Gentle pulse") {
                            vibrationPattern = "Gentle pulse"
                        }
                        selectionPill(title: "Steady reminder", subtitle: "One medium vibration", selected: vibrationPattern == "Steady reminder") {
                            vibrationPattern = "Steady reminder"
                        }
                        selectionPill(title: "Urgent alert", subtitle: "Three quick vibrations", selected: vibrationPattern == "Urgent alert") {
                            vibrationPattern = "Urgent alert"
                        }
                    }
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        Slider(value: $vibrationIntensity, in: 1...5, step: 1)
                            .tint(PulzPalette.petrol)
                    }
                ...
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    HStack(spacing: 10) {
                        toneCard(title: "Warm & nurturing", subtitle: "\"You're doing beautifully. This too shall pass.\"", selected: selectedTone == "Warm & nurturing") {
                            selectedTone = "Warm & nurturing"
                        }
                        toneCard(title: "Clear & direct", subtitle: "\"Urge detected. Time to pause and breathe.\"", selected: selectedTone == "Clear & direct") {
                            selectedTone = "Clear & direct"
                        }
                        toneCard(title: "Curious & gentle", subtitle: "\"Interesting moment. What's happening inside?\"", selected: selectedTone == "Curious & gentle") {
                            selectedTone = "Curious & gentle"
                        }
                    }
                    ...
                    HStack(spacing: 10) {
                        Text("Intervention language")
                            ...
                            .foregroundStyle(PulzPalette.soft)
                        Spacer()
                        Picker("Language", selection: $interventionLanguage) {
