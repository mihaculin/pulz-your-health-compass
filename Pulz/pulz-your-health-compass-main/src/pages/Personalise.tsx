...
/* ────────────── Themes ────────────── */
const themes = [
  { id: "aqua-bloom", name: "Aqua Bloom", sub: "Calm waters", from: "#b3ecec", to: "#D7C9DB", aqua: "#b3ecec", lavender: "#D7C9DB" },
  { id: "rose-mist", name: "Rose Mist", sub: "Soft warmth", from: "#F4D4E4", to: "#D7C9DB", aqua: "#F4D4E4", lavender: "#D7C9DB" },
  { id: "sky-pearl", name: "Sky Pearl", sub: "Clear sky", from: "#C8E8F4", to: "#b3ecec", aqua: "#C8E8F4", lavender: "#b3ecec" },
  { id: "forest-dew", name: "Forest Dew", sub: "In the woods", from: "#D4ECCC", to: "#b3ecec", aqua: "#D4ECCC", lavender: "#b3ecec" },
];
...
export default function Personalise() {
  ...
  /* Section A — Theme */
  const [selectedTheme, setSelectedTheme] = useState("aqua-bloom");
  const [customAccent, setCustomAccent] = useState("#b3ecec");

  const applyThemeVars = (aqua: string, lavender: string) => {
    ...
    document.documentElement.style.setProperty("--color-aqua", toHsl(aqua));
    document.documentElement.style.setProperty("--color-lavender", toHsl(lavender));
  };

  const selectTheme = (t: typeof themes[0]) => {
    setSelectedTheme(t.id);
    setCustomAccent(t.aqua);
    applyThemeVars(t.aqua, t.lavender);
  };

  const handleCustomAccent = (hex: string) => {
    setCustomAccent(hex);
    setSelectedTheme("");
    applyThemeVars(hex, "#D7C9DB");
  };
  ...
  const insertEmoji = (idx: number, emoji: string) => {
    ...
    setMessages((prev) => prev.map((m, i) => (i === idx ? m + emoji : m)));
    inputRefs.current[idx]?.focus();
  };

  const resetMessages = () => setMessages(defaultMessages.map((m) => m.text));
  ...
  const handleSave = () => {
    toast({
      description: "Your PULZ is personalised 💚",
      className: "bg-white border-l-2 border-l-[#b3ecec]",
    });
  };
