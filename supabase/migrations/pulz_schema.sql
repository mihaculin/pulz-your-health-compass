-- ============================================================
--  PULZ — Complete Schema Migration
--  Run this in your Supabase SQL Editor (new project)
--  https://syjwjytttzxeeehwkulo.supabase.co
-- ============================================================
...
-- Role check (security definer so RLS can call it safely)
...
CREATE POLICY "Specialists can view client profiles" ON public.profiles FOR SELECT USING (public.has_role(auth.uid(), 'specialist'));
...
CREATE TABLE public.client_profiles (
  ...
  subscription_tier            TEXT DEFAULT 'free'
                                 CHECK (subscription_tier IN ('free', 'premium', 'clinic')),
...
CREATE POLICY "Clients can view own client profile"            ON public.client_profiles FOR SELECT USING (auth.uid() = id);
...
CREATE POLICY "Specialists can view own profile"          ON public.specialist_profiles FOR SELECT USING (auth.uid() = id);
CREATE POLICY "Specialists can update own profile"        ON public.specialist_profiles FOR UPDATE USING (auth.uid() = id);
CREATE POLICY "Specialists can insert own profile"        ON public.specialist_profiles FOR INSERT WITH CHECK (auth.uid() = id);
CREATE POLICY "Clients can view their assigned specialist" ON public.specialist_profiles FOR SELECT USING (auth.uid() = ANY(client_ids));
...
CREATE TRIGGER update_personalisation_updated_at
...
-- ── INTERVENTION EVENTS (before risk_windows — circular FK) ──
CREATE TABLE public.intervention_events (
  id                  UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id             UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  risk_window_id      UUID,                          -- FK added after risk_windows
  sent_at             TIMESTAMPTZ,
  channel             TEXT,
...
-- Close the circular FK now that risk_windows exists
...
CREATE TABLE public.user_progress_snapshots (
  ...
  avg_urge_level         NUMERIC,
  binge_count            INTEGER DEFAULT 0,
  purge_count            INTEGER DEFAULT 0,
  meal_skipped_count     INTEGER DEFAULT 0,
  calm_days              INTEGER DEFAULT 0,
...
CREATE POLICY "Users can manage own progress snapshots" ON public.user_progress_snapshots
  FOR ALL USING (auth.uid() = user_id);

CREATE INDEX idx_progress_snapshots_user_date ON public.user_progress_snapshots (user_id, snapshot_date DESC);
...
CREATE POLICY "Users see own payments" ON public.payment_events
